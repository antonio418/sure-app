...
const dir = path.join(__dirname, 'public', 'presentacion');
...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .slide-container { width: 1280px; height: 720px; position: relative; overflow: hidden; background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); transform-origin: center center; }
...
slidesHTML.forEach((html, i) => {
    const pageNum = i + 1;
    const prevPage = pageNum > 1 ? (pageNum - 1) + '.html' : null;
    const nextPage = pageNum < 7 ? (pageNum + 1) + '.html' : null;
    
    // Auto scaling script so it looks like a real presentation
    const navScript = `
<script>
    // Scale presentation to fit window
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'ArrowRight' && '${nextPage}' !== 'null') window.location.href = '${nextPage}';
        if (e.key === 'ArrowLeft' && '${prevPage}' !== 'null') window.location.href = '${prevPage}';
    });
</script>
<div style="position: fixed; bottom: 20px; right: 20px; z-index: 9999; display: flex; gap: 10px;">
    ${prevPage ? \`<button onclick="window.location.href='${prevPage}'" style="background: rgba(255,255,255,0.1); color: #94a3b8; border: 1px solid rgba(255,255,255,0.2); padding: 8px 16px; border-radius: 8px; cursor: pointer; backdrop-filter: blur(4px); font-family: sans-serif; font-weight: bold; transition: all 0.3s;" onmouseover="this.style.background='rgba(255,255,255,0.2)'; this.style.color='white'" onmouseout="this.style.background='rgba(255,255,255,0.1)'; this.style.color='#94a3b8'"><i class="fas fa-arrow-left"></i> Anterior</button>\` : ''}
    ${nextPage ? \`<button onclick="window.location.href='${nextPage}'" style="background: #3b82f6; color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; box-shadow: 0 4px 15px rgba(59,130,246,0.4); font-family: sans-serif; font-weight: bold; transition: all 0.3s;" onmouseover="this.style.background='#2563eb'" onmouseout="this.style.background='#3b82f6'">Siguiente <i class="fas fa-arrow-right"></i></button>\` : ''}
</div>
`;

    const finalHtml = html.replace('NAV_SCRIPT', navScript);
    fs.writeFileSync(path.join(dir, pageNum + '.html'), finalHtml);
});

console.log('Slides created successfully in public/presentacion/');
