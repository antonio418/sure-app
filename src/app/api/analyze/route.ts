  ...
  try {
    ...
    if (email && process.env.RESEND_API_KEY) {
      try {
        ...
        let emailSubject = `SURE - Transactional Risk Assessment Report (Agent: ${agent?.toUpperCase()})`;
        ...
        if (agent === 'consolidator' && parsedJsonData) {
           const mailDict: Record<string, any> = {
             en: { subj: "SURE Transactional Risk Report", title: "SURE Ecosystem", sub: "B2B Transactional Risk Assessment", entity: "Audited Entity:", date: "Issue Date:", riskLbl: "Índice de Riesgo Transaccional", high: "HIGH", med: "MEDIUM", low: "LOW", anomTitle: "Points of Attention (Detected Discrepancies)", noAnom: "✅ No dynamic points of attention or risks were perceived in the provided documentation.", disclaimer: "LEGAL NOTICE: This report represents an eventual transactional risk index based strictly on the provided documentation at the time of analysis. If new or clarified documentation is obtained, this assessment can be updated by re-running the case. This document DOES NOT constitute a binding legal opinion. SURE Ecosystem disclaims any liability." },
             es: { subj: "SURE Reporte de Riesgo Transaccional", title: "Ecosistema SURE", sub: "Reporte de Riesgo Transaccional B2B", entity: "Entidad Analizada:", date: "Fecha de Emisión:", riskLbl: "Índice de Riesgo Transaccional", high: "ALTO", med: "MEDIO", low: "BAJO", anomTitle: "Puntos de Atención y Hallazgos", noAnom: "✅ No se percibieron puntos de atención ni eventuales riesgos en la documentación suministrada.", disclaimer: "AVISO LEGAL: Este reporte representa un eventual índice de riesgo de transacción basado únicamente en la información y documentos suministrados. Si el cliente obtiene mayor información o aclaratorias, se recomienda realizar una nueva evaluación para obtener un índice actualizado. Este documento NO constituye un dictamen legal vinculante. SURE Ecosystem se exime de cualquier responsabilidad." },
             fr: { subj: "SURE Rapport de Risque Transactionnel", title: "Écosystème SURE", sub: "Évaluation du Risque Transactionnel B2B", entity: "Entité Auditée:", date: "Date d'Émission:", riskLbl: "Indice de Risque Transactionnel", high: "ÉLEVÉ", med: "MOYEN", low: "FAIBLE", anomTitle: "Points d'Attention (Écarts Détectés)", noAnom: "✅ Aucun point d'attention ou risque n'a été détecté dans la documentation fournie.", disclaimer: "AVIS LÉGAL : Ce rapport représente un indice de risque transactionnel basé uniquement sur la documentation fournie. Si le client obtient des clarifications, il est recommandé de mettre à jour le document et de relancer l'analyse. Ce document NE constitue PAS un avis juridique contraignant. L'Écosystème SURE décline toute responsabilité." },
             de: { subj: "SURE Transaktionsrisikobericht", title: "SURE Ökosystem", sub: "B2B Transaktionsrisikobewertung", entity: "Geprüftes Unternehmen:", date: "Ausstellungsdatum:", riskLbl: "Transaktionsrisikoindex", high: "HOCH", med: "MITTEL", low: "NIEDRIG", anomTitle: "Auffälligkeiten (Erkannte Abweichungen)", noAnom: "✅ In den bereitgestellten Unterlagen wurden keine Auffälligkeiten oder Risiken festgestellt.", disclaimer: "RECHTLICHER HINWEIS: Dieser Bericht stellt einen Transaktionsrisikoindex dar, der ausschließlich auf den bereitgestellten Unterlagen basiert. Wenn neue Unterlagen vorliegen, kann die Bewertung aktualisiert werden. Dieses Dokument stellt KEIN verbindliches Rechtsgutachten dar. Das SURE Ökosystem lehnt jede Haftung ab." },
             pt: { subj: "SURE Relatório de Risco Transacional", title: "Ecossistema SURE", sub: "Avaliação de Risco Transacional B2B", entity: "Entidade Auditada:", date: "Data de Emissão:", riskLbl: "Índice de Risco Transacional", high: "ALTO", med: "MÉDIO", low: "BAIXO", anomTitle: "Pontos de Atenção (Desvios Detetados)", noAnom: "✅ Nenhum ponto de atenção ou risco foi detetado na documentação fornecida.", disclaimer: "AVISO LEGAL: Este relatório representa um índice de risco transacional baseado unicamente na documentação fornecida. Se o cliente obtiver esclarecimentos, é recomendável atualizar a documentação e reavaliar. Este documento NÃO constitui um parecer legal vinculativo. O Ecossistema SURE isenta-se de qualquer responsabilidade." },
           ...
           const scaleTitle = lang === 'es' ? "Índice de Riesgo Transaccional SURE" : "SURE Transactional Risk Scale Index";
           ...
           const scaleHighDesc = lang === 'es' ? "Discrepancias o desvíos significativos que requieren aclaratoria." : "Significant discrepancies or deviations requiring clarification.";
           const scaleCrit = lang === 'es' ? "Riesgo Crítico (76-100)" : "Critical Risk (76-100)";
           const scaleCritDesc = lang === 'es' ? "Inconsistencias críticas o licencia inactiva/vencida." : "Critical inconsistencies or inactive/expired commercial license.";
