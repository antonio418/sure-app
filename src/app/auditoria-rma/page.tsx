...
const localTranslations: Record<string, Record<string, string>> = {
  es: {
    ...
    modeComparative: 'Proyectos — Asistente Avanzado',
    ...
    cardProjectTitle: '2. Proyectos — Asistente Avanzado',
  ...
  en: {
    ...
    modeComparative: 'Projects — Advanced Assistant',
    ...
    cardProjectTitle: '2. Projects — Advanced Assistant',
...
export default function DocumentProcessorPage() {
  ...
  const handleResetPassword = async () => {
    if (!clientEmail.trim()) {
      setAuthError(language === 'es' ? 'Por favor ingrese su correo electrónico.' : 'Please enter your email.');
      return;
    }
    setAuthLoading(true);
    setAuthError(null);
  ...
  return (
    <main className="flex flex-col min-h-screen bg-[#0B192C] text-slate-100 selection:bg-emerald-500/30 font-sans overflow-x-hidden">
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-6 pt-32 pb-16 flex flex-col items-center">
        ...
        {/* Auth Step */}
        {workflowStep === 'auth' && (
          <div className="max-w-md mx-auto w-full px-6 py-12 text-left space-y-6 animate-fade-in bg-[#152338]/30 border border-white/15 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="text-center space-y-2">
              ...
              <h2 className="text-2xl font-black text-white uppercase tracking-tight">
                {language === 'es' ? 'Portal de Acceso' : 'Access Portal'}
              </h2>
              <p className="text-xs text-slate-400">
                {language === 'es' 
                  ? 'Inicia sesión o regístrate para comenzar la auditoría de riesgos.' 
                  : 'Log in or sign up to begin the risk auditing process.'}
              </p>
            </div>

            {/* Mode Switcher Tab */}
            {authMode !== 'forgot' && (
              <div className="flex bg-[#050a15] p-1 rounded-xl border border-white/5">
                <button
                  type="button"
                  onClick={() => {
                    setAuthMode('login');
            ...
            <form onSubmit={handleAuthSubmit} className="space-y-4">
                ...
                <>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-[#050a15] border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-emerald-500"
                    />
                  </div>

                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
                      {language === 'es' ? 'Nº de Registro Fiscal (Opcional)' : 'Tax ID / Registration (Optional)'}
                    </label>
                    <input
                      type="text"
                      value={taxId}
              ...
              {authMode === 'login' && (
                <>
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
                      {language === 'es' ? 'Contraseña (Dejar vacío para Magic Link)' : 'Password (Leave empty for Magic Link)'}
                    </label>
                    <input
                      type="password"
                      value={authPassword}
                  ...
                  <div className="flex justify-end mt-1">
                    <button
                      ...
                      className="text-[10px] text-slate-400 hover:text-emerald-400 transition-colors font-bold uppercase tracking-wider cursor-pointer"
                    >
                      {language === 'es' ? '¿Olvidó su usuario o clave secreta?' : 'Forgot your username or password?'}
                    </button>
                  </div>
                </>
              )}
              ...
              {authMode === 'forgot' && (
                <>
                  <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-2xl p-4 text-xs text-slate-300 leading-relaxed space-y-2">
                    <p>
                      {language === 'es'
                        ? '• Tu usuario es el correo electrónico con el que te registraste.'
                        : '• Your username is the email address you registered with.'}
                    </p>
                    <p>
                      {language === 'es'
                        ? '• Para restablecer tu clave secreta, ingresa tu correo abajo y te enviaremos un enlace de recuperación.'
                        : '• To reset your password, enter your email below and we will send you a recovery link.'}
                    </p>
                  </div>
              ...
              {authError && (
                <div className="bg-rose-500/10 border border-rose-500/30 rounded-xl p-3 flex items-start gap-2.5">
                  ...
                  <p className="text-xs text-rose-400 leading-normal">{authError}</p>
                </div>
              )}
              ...
              {authSuccessMsg && (
                <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-3 flex items-start gap-2.5">
                  ...
                  <p className="text-xs text-emerald-400 leading-normal">{authSuccessMsg}</p>
                </div>
              )}
              ...
              <button
                ...
                className="w-full py-3 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-55 text-black font-black text-xs uppercase tracking-widest rounded-xl hover:scale-[1.01] transition-transform cursor-pointer flex items-center justify-center gap-2"
              >
                {authLoading ? (
                  <Loader2 className="w-4 h-4 animate-spin text-black" />
                ) : authMode === 'login' ? (
                  authPassword ? (language === 'es' ? 'Entrar con Contraseña' : 'Log In with Password') : (language === 'es' ? 'Recibir Enlace Mágico' : 'Request Magic Link')
                ) : authMode === 'signup' ? (
                  (language === 'es' ? 'Crear Cuenta y Continuar' : 'Create Account & Continue')
                ) : (
              ...
              {authMode === 'forgot' ? (
                <button
                  ...
                  className="w-full py-2.5 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer"
                >
                  {language === 'es' ? 'Volver a Conectarse' : 'Back to Log In'}
                </button>
              ) : (
                <>
                  <div className="relative my-4 flex items-center justify-center">
                    <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-white/10" /></div>
                    <span className="relative px-3 bg-[#132032] text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-3 cursor-pointer"
                  >
