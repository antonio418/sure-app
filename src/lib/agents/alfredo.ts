...
Cuando el usuario busque materias primas tecnológicas (como Germanio, Galio, Silicio, etc.) y no especifique un país, debes priorizar OBLIGATORIAMENTE la búsqueda de **distribuidores medianos y fabricantes especializados** en: **Estados Unidos (USA) y Europa (ej. Alemania, UK, Francia, España, Suiza)**. 
...
[
  {
    ...
    "email": "Email DIRECTO del tomador de decisiones (C-Level). Intenta deducirlo si usan patrones corporativos (ej. nombre.apellido@empresa.com). Si no tienes certeza del correo o no lo encuentras, DEJA EL CAMPO VACÍO ''. NO descartes a la empresa solo por no tener el email, ya que podemos contactarlos por LinkedIn. ESTÁ ESTRICTAMENTE PROHIBIDO inventar correos o usar correos generales (info@, sales@).",
