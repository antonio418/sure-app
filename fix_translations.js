...
const filePath = 'src/lib/uiTranslations.ts';
...
for (const line of lines) {
    new_lines.push(line);
    const lang_match = line.match(/^  ([a-z]{2}): \{/);
    if (lang_match) {
        current_lang = lang_match[1];
    }
        
    if (line.includes('intake_lbl_lang:') && current_lang && translations[current_lang]) {
        const t = translations[current_lang];
        new_lines.push(`      intake_context_title: "${t['intake_context_title'].replace(/"/g, '\\"')}",`);
        new_lines.push(`      intake_context_sub: "${t['intake_context_sub'].replace(/"/g, '\\"')}",`);
        new_lines.push(`      intake_context_placeholder: "${t['intake_context_placeholder'].replace(/"/g, '\\"')}",`);
    }
}

fs.writeFileSync(filePath, new_lines.join('\n'), 'utf-8');
console.log('Successfully updated uiTranslations.ts');
